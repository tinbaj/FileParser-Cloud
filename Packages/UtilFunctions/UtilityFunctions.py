...
def writeFile(strBuffer: str, strInput: str):
...
def createReturnDict(Return_Status:int,Parsing_Status:bool = False, Record_count:int = 0,outputFile:str = None,errorMessage:str = None):
    """
    :param Return_Status:
    ...
    if outputFile:
        returnDict['outputFile'] = outputFile
    if errorMessage:
...
def csv_parser(fname, *, delimiter=',', quotechar='"', include_header=False):
    ...
    if not include_header:
        reader = csv.reader(fname.split('\n')[1:], delimiter=delimiter, quotechar=quotechar)
    else:
