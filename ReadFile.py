...
class ReadFile:
    ...
    def file_tokenizer_csv(fileName,funcInputs: dict):
        ...
        try:
            ...
            tmpFileName='tmp.csv'
            outputFileName = os.path.join(file, 'OUTPUT', 'CSV','CSV_' + datetime.now().strftime('%d_%m_%y_%H_%M_%S') + "_output.csv")
            outputStringBuffer = ""
            FieldIndex = []
            ...
            headerList = Packages.extract_field_names(tmpFileName, delimiter=delimiter)
            ...
            if len(FieldIndex) == 1:
                outputStringBuffer = Packages.writeFile(outputFileName, outputStringBuffer, headerrow)
            else:
                outputStringBuffer = Packages.writeFile(outputFileName,outputStringBuffer,delimiter.join(headerrow))
            csvdata = Packages.Extract_data(tmpFileName, delimiter=delimiter)
            ...
            for t in csvdata:
                ...
                print(row)
                if len(FieldIndex) == 1:
                    outputStringBuffer = Packages.writeFile(outputFileName, outputStringBuffer, row)
                else:
                    outputStringBuffer = Packages.writeFile(outputFileName, outputStringBuffer, delimiter.join(row))
            ...
            if not writeFile.closed:
                writeFile.close()
            Packages.writeLogFile("Exiting function file_tokenizer_csv(fileName: str,funcInputs: dict) of Module: " + __name__)
        return Packages.createReturnDict(Return_Status=200, Parsing_Status=Packages.sv.XL_SUCCESS,Record_count=ReadFile.recCounter, outputFileName=outputFileName)
    ...
    def file_tokenizer_txt(fileName,funcInputs: dict):
        ...
        try:
            ...
            for RecordText in Packages.getRecordRegex(file_contents, recordMarkerStartText, recordMarkerEndText):
                ReadFile.recCounter += 1
                headfieldList = []
                tableFieldList = []
                ...
                if not headerPrinted:
                    OutputFileBuffer = Packages.writeFile(writeFile, OutputFileBuffer, reportHeader)
                ...
                for table in Packages.getRecordRegex(RecordText, tableMarkerStartText, tableMarkerEndText):
                    ...
                    if tableHeaderHasUnits:
                        if headerPrinted:
                            ...
                            OutputFileBuffer = Packages.writeFile(writeFile, OutputFileBuffer,
    ...
    def file_tokenizer_xml(fileName, funcInputs: dict):
        ...
        try:
            ...
            outputStringBuffer = Packages.writeFile(writeFile, outputStringBuffer, headerRecord)
                ...
                if len(headerRecord) == len(recordHeader):
                    ...
                    while (indexHeader < len(headerRecord.split(','))):
                        if tableHeader[indexRecord] == headerRecord.split(',')[indexHeader]:
                            ...
                            strRecord += ','
                            indexHeader += 1
                outputStringBuffer = Packages.writeFile(writeFile, outputStringBuffer, strRecord)
